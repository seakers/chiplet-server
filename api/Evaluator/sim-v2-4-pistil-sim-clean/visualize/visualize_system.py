        ...
        for lr in range(2):
            ...
            for i in range(2):
                if self.banks_per_group == 1:
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("A" if i == 0 else "B")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                ...
                if self.banks_per_group == 2:
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("A" if i == 0 else "B")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+self.bank_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.y_ctrl_height-2*BUFFER_VIS], label="Y-CTRL" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+self.bank_height+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("A" if i == 0 else "B")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                
                if self.banks_per_group == 3:
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("A" if i == 0 else "B")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+self.bank_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("A" if i == 0 else "B")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+self.bank_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.y_ctrl_height-2*BUFFER_VIS], label="Y-CTRL" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+self.bank_height*2+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("A" if i == 0 else "B")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                ...
                if self.banks_per_group == 4:
                    ...
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+self.bank_height*2+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("A" if i == 0 else "B")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+i*bank_group_height+self.bank_height*3+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("A" if i == 0 else "B")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                ...
                if self.banks_per_group == 1:
                    ...
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                
                if self.banks_per_group == 2:
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    ...
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+self.bank_height+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                
                if self.banks_per_group == 3:
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+self.bank_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.y_ctrl_height-2*BUFFER_VIS], label="Y-CTRL" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+self.bank_height+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+self.bank_height*2+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                
                if self.banks_per_group == 4:
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+self.bank_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    ...
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+self.bank_height*2+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
                    self.add_box(ax, [x_origin+lr*(self.mem_shoreline-self.bank_width)+BUFFER_VIS, y_origin+offset+i*bank_group_height+self.bank_height*3+self.y_ctrl_height+BUFFER_VIS], [self.bank_width-2*BUFFER_VIS, self.bank_height-2*BUFFER_VIS], label=f"BANK GROUP {("C" if i == 0 else "D")}" if text else "", facecolor=MEMORY_COLOR, edgecolor=OUTLINE_COLOR)
