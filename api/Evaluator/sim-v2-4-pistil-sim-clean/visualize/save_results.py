...
def save_results(all_results, sys_manager, llm_manager, args, csv_filename, verbose=True):
    ...
    if verbose:
        ...
        # Extract values to avoid f-string syntax errors
        system_compute = stats["system_compute_TOPS"]
        system_bw = stats["system_bandwidth_TBps"]
        system_cap = stats["system_capacity_GB"]
        avg_power = stats["average_power_W"]
        print(f"System Compute:\t\t{system_compute:.2f} TOPs")
        print(f"System Memory BW:\t{system_bw:.2f} TB/s")
        print(f"System Memory Cap:\t{system_cap:.2f} GB")
        print(f"System Avg. Power:\t{num_cus*avg_power:.2f} W [for this application]")
        print("")
        print("Core Configuration")
        tmacs_per_core = stats["tmacs_per_core"]
        mem_buffer = stats["mem_buffer_size"]
        net_buffer = stats["net_buffer_size"]
        print(f"Num TMACs/Core:\t\t{tmacs_per_core}")
        print(f"Mem Buffer/Core:\t{(mem_buffer/(1024*1024))} MB")
        print(f"Net Buffer/Core:\t{(net_buffer/(1024*1024))} MB")
        print("")
        print("Memory Configuration")
        hblc_ranks = stats["hblc_ranks"]
        hblc_bank_groups = stats["hblc_bank_groups"]
        hblc_frac = stats["hblc_frac_bank_cap"]
        print(f"Num Ranks:\t\t{hblc_ranks}")
        print(f"Num Banks / Bank Group:\t{hblc_bank_groups}")
        print(f"Fraction Bank Capacity:\t{hblc_frac}")
        print("###########################################")
        print("Application Configuration")
        model_name = stats["model_name"]
        batch_size = stats["batch_size"]
        prefill_batch = stats["prefill_batch"]
        prefill_chunk = stats["prefill_chunk_size"]
        prefill_cached = stats["prefill_cached"]
        kv_cache = stats["kv_cache"]
        sim_num_layers = stats["sim_num_layers"]
        num_layers = stats["num_layers"]
        sim_lm_head = stats["sim_lm_head"]
        total_model_cap = stats["total_model_capacity_GB"]
        w_dtype = stats["w_dtype_B"]
        kv_dtype = stats["kv_dtype_B"]
        print(f"Model:\t\t\t{model_name}")
        print(f"Batch Size:\t\t{batch_size}")
        print(f"Prefill Batch:\t\t{prefill_batch}")
        if prefill_batch:
            print(f"Prefill Chunk Size:\t{prefill_chunk}")
            print(f"Prefill Cached Seq Len:\t{prefill_cached}")
        print(f"KV Cache:\t\t{kv_cache}")
        print(f"Sim Num Layers:\t\t{sim_num_layers} / {num_layers}")
        if sim_num_layers != num_layers or sim_num_layers != -1:
            print("\t\t\t[WARNING] Sim layers not same as all layers, scaling latency and energy assuming same behavior")
        print(f"Sim LM Head:\t\t{sim_lm_head} [Vocab projection layer at end?]")
        if total_model_cap < system_cap:
            print(f"\t\t\tCapacity Satisfied - Enough capacity for model and KV cache")
        else:
            print(f"[WARNING] Capacity Not Satisfied ({total_model_cap} !< {system_cap}) Not enough capacity for model and KV cache)")
        print(f"Bytes per Weight Param:\t{w_dtype}")
        print(f"Bytes per KV$ Param:\t{kv_dtype}")
        print("###########################################")
        print("General Performance Metrics")
        print(f"Latency per Token\t{latency_ms:.2f} ms")
        print(f"Energy per Inference\t{epi:.2f} mJ (BS={batch_size})")
        print(f"Energy per Token\t{epi/batch_size:.2f} mJ")
        print(f"Energy per Token\t{epi/args.sim_batch_size:.2f} mJ (w/ Prefill)")
        print(f"Average System Power\t{num_cus*avg_power:.2f} W")
        system_cost = stats["system_cost"]
        print(f"Estimated System Cost\t${system_cost:.2f} (Area Based)")
        print("###########################################")
        print("Specific HW Metrics")
        avg_comp_util = stats["avg_comp_util"]
        avg_mem_util = stats["avg_mem_util"]
        avg_cache_util = stats["avg_cache_util_MB"]
        peak_buffer = stats["peak_buffer_MB"]
        cores_per_cu = stats["cores_per_cu"]
        avg_power_comp = stats["average_power_comp_W"]
        avg_power_mem = stats["average_power_mem_W"]
        avg_power_net = stats["average_power_net_W"]
        print(f"Avg. Compute Util.\t{100*avg_comp_util:.2f} %")
        print(f"Avg. Memory BW Util.\t{100*avg_mem_util:.2f} %")
        print(f"Memory Capacity Util.\t{100*total_model_cap / system_cap:.2f} %")
        print(f"Avg. Mem. Buffer Util.\t{100*avg_cache_util/(mem_buffer*cores_per_cu/(1024*1024)):.2f} %")
        print(f"Peak Mem. Buffer Util.\t{100*peak_buffer/(mem_buffer*cores_per_cu/(1024*1024)):.2f} %")
        print(f"Avg. Sys Power Compute\t{num_cus*avg_power_comp:.2f} W")
        print(f"Avg. Sys Power Memory\t{num_cus*avg_power_mem:.2f} W")
        print(f"Avg. Sys Power Network\t{num_cus*avg_power_net:.2f} W")
        print("###########################################")
