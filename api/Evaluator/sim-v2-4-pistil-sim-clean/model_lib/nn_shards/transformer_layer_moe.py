...
class TransformerLayerMoE:
    def __init__(self, parent, layer_id, model_dim, ff_dim, num_local_experts, num_experts_per_tok, head_dim, num_heads, kv_heads, max_seq_len, cos, sin, rms_norm_eps, act_fn, attn_softcap, q_pre_attn_scalar, dtype):
        self.parent             = parent
        ...
        # shared expert
        self.gate_proj_s        = LinearLayer(self.parent, self.layer_id, self.model_dim, self.ff_dim, self.input_dtype)
        self.up_proj_s          = LinearLayer(self.parent, self.layer_id, self.model_dim, self.ff_dim, self.input_dtype)
        self.gate_up_proj_s     = LinearLayer(self.parent, self.layer_id, self.model_dim, 2*self.ff_dim, self.input_dtype)
        self.down_proj_s        = LinearLayer(self.parent, self.layer_id, self.ff_dim, self.model_dim, self.input_dtype)
        
        # routed expert
        self.expert_proj        = LinearLayer(self.parent, self.layer_id, self.model_dim, self.num_local_experts, self.input_dtype)
        self.gate_proj_e        = LinearLayer(self.parent, self.layer_id, self.model_dim, self.ff_dim, self.input_dtype)
        self.up_proj_e          = LinearLayer(self.parent, self.layer_id, self.model_dim, self.ff_dim, self.input_dtype)
        self.gate_up_proj_e     = LinearLayer(self.parent, self.layer_id, self.model_dim, 2*self.ff_dim, self.input_dtype)
        self.down_proj_e        = LinearLayer(self.parent, self.layer_id, self.ff_dim, self.model_dim, self.input_dtype)
