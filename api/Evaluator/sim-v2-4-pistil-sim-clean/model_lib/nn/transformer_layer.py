...
class TransformerLayer:
    def __init__(self, my_model, layer_id, model_dim, ff_dim, head_dim, num_heads, kv_heads, max_seq_len, cos, sin, rms_norm_eps, act_fn, attn_softcap, q_pre_attn_scalar, dtype):
        ...
        self.gate_proj          = LinearLayer(self.my_model, self.layer_id, self.model_dim, self.ff_dim, self.input_dtype)
        self.up_proj            = LinearLayer(self.my_model, self.layer_id, self.model_dim, self.ff_dim, self.input_dtype)
        self.gate_up_proj       = LinearLayer(self.my_model, self.layer_id, self.model_dim, 2*self.ff_dim, self.input_dtype)
        self.down_proj          = LinearLayer(self.my_model, self.layer_id, self.ff_dim, self.model_dim, self.input_dtype)
    ...
    def init_random(self, use_additional_norms=False):
        ...
        self.gate_proj.init_random()
        self.up_proj.init_random()
        self.gate_up_proj.init_random()
        self.down_proj.init_random()
    ...
    def forward(self, layer_input, layer_id, use_base=False):
        ...
        if use_base:
            ...
            hidden_states   = self.pre_ff_norm.forward_weights(layer_sum)
        ...
        #################################################################
        # gate and up proj
        hidden_states_gate  = self.gate_proj.forward(hidden_states)
        hidden_states_up    = self.up_proj.forward(hidden_states)
        #################################################################
        ...
        if not use_base:
            hidden_states_gate   = self.pre_ff_norm.forward_var(hidden_states_gate)
            hidden_states_up     = self.pre_ff_norm.forward_var(hidden_states_up)
        ...
        self.save_state.save_state(layer_id, name="act", param=hidden_states)


        #################################################################
        # mlp down proj
        hidden_states   = self.down_proj.forward(hidden_states)
        self.save_state.save_state(layer_id, name="down", param=hidden_states)
        ##################################################################
